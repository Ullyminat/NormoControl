...
**Для Преподавателей**
- **Интерактивный Конструктор Стандартов**: Создание переиспользуемых шаблонов с помощью Drag&Drop зон, мгновенной обратной связи и обучающего "Героя-помощника" (Tutorial Hero).
...
- **Интерактивные Отчеты (Report Modal)**: Красивые всплывающие окна с плавной анимацией расширения, показывающие детальный анализ каждой работы.
- **Продвинутая Панель Аналитики**: Мониторинг студентов с "роллирующими счетчиками" (SlotCounters) для оценок и красивыми графиками.
- **Изоляция и Безопасность Данных**: Полное разделение стандартов и работ на базе Role-Based Access Control (RBAC).

**Для Студентов**
- **Премиальный Интерфейс**: Интерактивный UI с 3D-параллаксом стеклянных карточек (Glassmorphism), магнитной физикой кнопок и идеальной плавностью (60+ FPS благодаря GSAP).
- **Мгновенная Проверка и Физика Файлов**: Загрузка документов перетаскиванием с эффектами "сквош и стретч" (Squash & Stretch).
- **Визуальное Картирование Ошибок**: Интерактивный просмотрщик PDF с точным выделением ошибок и плавными переходами камеры.
- **Детальные Отчеты**: Всеобъемлющий разбор нарушений с анимацией "Particle Spark" при 100% результате.
- **История Проверок**: Доступ ко всем предыдущим проверкам с полными отчетами.
...
| **Фронтенд** | React 18 (Vite) | Современное SPA с горячей перезагрузкой модулей |
| **Анимации & Физика** | GSAP (GreenSock) | 3D Трансформации, Параллакс, MorphSVG, CustomEases ("Disney Physics") |
| **Парсер Документов** | Нативный парсинг XML | Потоковый анализ DOCX без внешних зависимостей (LibreOffice не нужен) |
| **Рендеринг PDF** | pdf.js | Клиентский просмотр PDF и извлечение текста для позиционирования ошибок |
| **База Данных** | SQLite 3 | Встроенная БД с GORM для развертывания без сложной конфигурации |
| **Аутентификация & Security** | JWT (HTTP-only cookies), RBAC | Максимальная защита: Rate Limiting, CORS хедеры, изоляция ролей |
| **Развертывание** | Docker Compose | Контейнеризированные микросервисы с Nginx |
...
├── backend/
│   ├── cmd/
│   │   ├── server/main.go   # Основной API-сервер
│   │   └── seeder/main.go   # Сидер тестовых данных
│   ├── internal/
│   │   ├── ai/              # Интеграция с Gemini API
│   │   ├── auth/            # Логика регистрации и JWT
│   │   ├── checker/         # Движок анализа DOCX
│   │   ├── database/        # Слой данных (SQLite/GORM)
│   │   ├── handlers/        # HTTP-контроллеры
│   │   ├── middleware/      # CORS, RBAC, Auth middleware
│   │   └── models/          # GORM-модели данных
│   ├── fonts/               # Шрифты для конвертации (GOST Type B)
│   └── uploads/             # Хранилище документов и PDF
│
├── frontend/
│   ├── src/
...
│   │   │   ├── auth/        # Авторизация
│   │   │   ├── common/      # Общие компоненты (Layouts)
│   │   │   ├── dashboard/   # Интерфейс студента
│   │   │   ├── student/     # Просмотр отчетов и PDF
│   │   │   └── teacher/     # Редактор стандартов и статистика
│   │   ├── store/           # Состояние (Zustand)
│   │   └── api/             # Клиент API (Axios)
│   └── public/              # Статика и шрифты
│
...
Перед запуском проекта необходимо настроить переменные окружения для обеспечения безопасности (JWT и CORS).

1. Перейдите в папку `backend/`
2. Скопируйте пример конфигурации (или создайте файл `.env`):
   ```bash
   cp .env.example .env
   ```
3. Убедитесь, что в `backend/.env` заданы следующие обязательные параметры:
   ```env
   # Секрет для подписи токенов авторизации (ОБЯЗАТЕЛЬНО ИЗМЕНИТЬ В ПРОДЕ!)
   JWT_SECRET=your_super_secret_key_change_me
   
   # Разрешенный домен для CORS (например, http://localhost:5173 для dev)
